    ...
    def create_model(model_type: str) -> nn.Module:
        ...
        if 'dino' in model_type:
            ...
            if model_type == 'dino_vits8':
                ...
                state_dict = torch.load('./weights/dino_deitsmall8_pretrain.pth', map_location='cpu')
                model.load_state_dict(state_dict, strict=True)
            elif model_type == 'dino_vits16':
                model = vits.__dict__['vit_small'](patch_size=16, num_classes=0)
                state_dict = torch.load('./weights/dino_deitsmall16_pretrain.pth', map_location='cpu')
                model.load_state_dict(state_dict, strict=True)
            elif model_type == 'dino_vitb8':
                model = vits.__dict__['vit_base'](patch_size=8, num_classes=0)
                state_dict = torch.load('./weights/dino_vitbase8_pretrain.pth', map_location='cpu')
                model.load_state_dict(state_dict, strict=True)
            elif model_type == 'dino_vitb16':
                model = vits.__dict__['vit_base'](patch_size=16, num_classes=0)
                state_dict = torch.load('./weights/dino_vitbase16_pretrain.pth', map_location='cpu')
                model.load_state_dict(state_dict, strict=True)
            else:
